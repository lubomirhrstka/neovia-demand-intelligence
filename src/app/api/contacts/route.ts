...
export async function POST(request: Request) {
  ...
  if (!body.firstName || !body.lastName || !body.company || (!body.email && !body.phone)) return NextResponse.json({ error: "Doplňte jméno, firmu a alespoň e-mail nebo telefon." }, { status: 400 });
  const db = getDb();
  const matchRules = [];
  if (body.email) matchRules.push(eq(contacts.email, body.email.trim().toLowerCase()));
  if (body.phone) matchRules.push(eq(contacts.phone, body.phone.trim()));
  ...
  const [created] = await db.insert(contacts).values({ firstName: body.firstName, lastName: body.lastName, role: body.role || null, email: body.email || null, phone: body.phone || null, source: sourceTag, companyId: company.id, ownerId: user.id }).returning();
...
export async function PATCH(request: Request) {
  ...
  if (!user) return NextResponse.json({ error: "Nepřihlášený uživatel" }, { status: 401 });
  const body = await request.json();
  if (!body.id || !body.firstName || !body.lastName || !body.company || (!body.email && !body.phone)) return NextResponse.json({ error: "Doplňte jméno, firmu a alespoň e-mail nebo telefon." }, { status: 400 });
  const db = getDb();
  ...
  if (email) matchRules.push(eq(contacts.email, email));
  if (phone) matchRules.push(eq(contacts.phone, phone));
  const [duplicate] = matchRules.length ? await db.select().from(contacts).where(and(eq(contacts.ownerId, user.id), or(...matchRules))).limit(1) : [];
  ...
  const company = existingCompany || (await db.insert(companies).values({ name: body.company, source: sourceTag, ownerId: user.id }).returning())[0];
  if (existingCompany && !existingCompany.source) await db.update(companies).set({ source: sourceTag, updatedAt: new Date() }).where(eq(companies.id, existingCompany.id));
  const [updated] = await db.update(contacts).set({ firstName: body.firstName, lastName: body.lastName, role: body.role || null, email, phone, source: sourceTag, companyId: company.id, verified: Boolean(body.verified), updatedAt: new Date() }).where(and(eq(contacts.id, body.id), eq(contacts.ownerId, user.id))).returning();
